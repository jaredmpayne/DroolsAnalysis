...
public class Drools {
    ...
    public static let rules = [
        ...
        when
            $p: EndSystem($labels: labels contains "APPLE TV");  // Currently no host is an apple tv, therefore no rule should be generated
            $i: NetInterface(hostedBy == $p);
            $l: Link ( $i == src || == dst);
        then
            String cfg = ConfigGen.fromEndSystemLink($l,
        ...
        then
            List<NetDevice> path = topo.computePath($ids, $h1)  // should return list of switches
            String pol = ConfigGen.pathConfig(path,
                 PolicyAction.ROUTE,
